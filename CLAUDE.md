...
**V245 (2026-07-10) RunBook-style state cards** (user-requested redesign): a card's dominant state paints the WHOLE card — 3px left accent edge + gradient wash (`st-eo` red > `st-skip` slate > `st-shared` green [shake excluded — shared-by-default there, not a state] > `st-lo` amber > `st-cook` amber > `st-set` purple). `accCls` in `buildMealCardHtml` mirrors the pill conditions exactly; eat-out card hardcodes `st-eo`; Shared tab passes `st-lo`/`st-cook` only (green would be uniform noise there). **Pill visibility is pure CSS** — `.mc:not(.open):not(.skipped):not(.st-eo) .mc-pill:not(.on){display:none}`: collapsed cards show only active (`.on`) pills; expanding (`toggleCard` flips `.open`, no re-render) reveals the full toggle row (Shared/Skip/Eat Out/⇄). Skipped + eat-out cards keep their pill pair visible — they can't expand (mc-meta inert/absent) and the skip→eat-out flow needs both reachable. "⤓ Here" carries `.on` so move destinations stay visible while collapsed.
...
- **3-zone cell**: left (Him), center (cell-wide/shared), right (Her). Tapping a zone cycles its state.
- **Every first tap clears (V85)**: a tap on a side that already has visual state clears it before advancing (`_v85ClearSideVisualIfAny` unifies link/shared/eo/skip/manual-purple clears).
...
## Current State (2026-07-10, V247 — `main` @ tip)
...
- **Branch tip**: `MP_2026-07-10_V10` == `main` (V247). Convention: every commit gets an `MP_<date>_V<N>` branch AND `main` fast-forwarded to it; push both immediately.
